...
public class Math3 {
	...
	static void returnChange() {
		...
		System.out.println(moneyYouGive); // i print it to find out how the change acts
		System.out.println("You should get " + change); // i print it to find out how the change acts
		for (int i = 0; i < moneyLength; i++) {
			while (change >= money[i]) {
				System.out.println("now change is: " + change); // i print it to find out how the change acts
				change -= ((int)100 *money[i]) / 100;
				System.out.println("and now it's:" + change);  // i print it to find out how the change acts
				amount[i] += 1;
